...
void modifier_lumin(image_t *src, float pourcent)
{
  float max = 256.0;
  for (int i = 0; i<(src->w*src->h);i++){
    src->buff[i] = ((src->buff[i]*pourcent) % max);
  }
}
...
int main(void) {
  ...
  image_t * image = charger_image_pgm("vache.pgm");
  //negatif(image);
  float lum = 2.0;
  modifier_lumin(image, lum);
  int res = sauver_image_pgm("vache_lum_2.pgm",image);
